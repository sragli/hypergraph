defmodule Hypergraph.CorrelationLength do
  @moduledoc """
  ...
  def compute(hypergraph, max_distance \\ 10, region_size \\ 5, samples \\ 100) do
    ...
    mutual_info_by_distance =
      ...
      |> Task.async_stream(
        fn d ->
          {d,
           calculate_mutual_info_at_distance(vertex_set, adjacency_map, d, region_size, samples)}
        end,
        ordered: false,
        timeout: :infinity
      )
    ...
    fit_exponential_decay(mutual_info_by_distance)
  end
  ...
  defp calculate_mutual_info_at_distance(
         vertex_set,
         adjacency_map,
         distance,
         region_size,
  ...
  defp sample_region_pair_at_distance(vertex_set, adjacency_map, target_distance, region_size) do
    ...
    distant_vertices =
      find_vertices_at_distance_bfs(region1, adjacency_map, target_distance, vertex_set)
  ...
  defp bfs_distance_search(
    ...
    cond do
      distance == target_distance ->
        ...
        bfs_distance_search(rest, visited, adjacency_map, target_distance, all_vertices, [
          current | result
        ])
  ...
  defp extract_region_features(region, adjacency_map) do
    ...
    |> Enum.map(fn vertex ->
      ...
      degree =
        case Map.get(adjacency_map, vertex) do
          nil -> 0
